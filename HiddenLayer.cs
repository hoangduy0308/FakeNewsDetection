...
namespace DeepLearningSharp
{
    public class HiddenLayer
    {
        ...
        public HiddenLayer(int N, int n_in, int n_out, double[][] W, double[] b, Random rng, String activation)
        {
            ...
            if (W == null)
            {
                this.W = utils1.CreateCrossArray<double>(n_out, n_in);// new double[n_out,n_in];
            ...
            if (activation == "sigmoid" || activation == null)
            {
                ...
                this.activation = utils1.sigmoid;
                this.dactivation = utils1.dsigmoid;
            ...
            else if (activation == "tanh")
            {
                ...
                this.activation = utils1.tanh;
                this.dactivation = utils1.dtanh;
            }
            else if (activation == "ReLU")
            {
                //this.activation = (double x) -> utils.ReLU(x);
                //this.dactivation = (double x) -> utils.dReLU(x);
                this.activation = utils1.ReLU;
                this.dactivation = utils1.dReLU;
            }
            else
            {
        ...
        public int[] dropout(int size, double p, Random rng)
        {
            ...
            {
                mask[i] = utils1.binomial(1, p, rng);
            }
